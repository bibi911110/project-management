...
import './style.css';
import { Issue, updateIssue, getIssuesByProjectId, getIssue } from '../../utils/API/issue_API';
...
class Swimlane extends React.Component<SwimlaneProps, SwimlaneState> {
    ...
    fetchIssuesAndKanbanTypes = async () => {
        ...
        if (success) {
            ...
            const { data: issues, success: isSuccess } = await getIssuesByProjectId(currentProject._id);
            if (isSuccess) {
                let state = {};
                kanbanTypes.forEach((id) => {
                    state = {
                        ...state,
                        [id]: issues.filter((issue) => issue.statusId === '' || issue.statusId === id),
                    };
                });
        ...
        (this.state.kanbanTypes as KanbanType[]).forEach((type) => {
            state = {
                ...
                [type._id]: this.state[type._id].filter(
                    (issue: Issue) => issue.creatorId === currentUser._id || issue.assigneeId
                ),
            };
        });
    ...
    onDragEnd = (result: DropResult) => {
        ...
        if (sourceDropId === destinationDropId) {
            const items = this.reorder(this.state[sourceDropId], source.index, destination.index);
            ...
            const result = this.move(this.state[sourceDropId], this.state[destinationDropId], source, destination);
            this.setState(
                {
    ...
    updateIssue = async (issueId: string) => {
        ...
        if (success) {
            const issueIndex = this.state[data.statusId].findIndex((issue) => issue._id === issueId);
