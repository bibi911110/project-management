...
const Row: React.FC<RowProps> = (props) => {
    ...
    const getIssueType = (issue: Issue) => {
        const issueType = issueTypes.find((issueType) => issueType._id === issue.issueTypeId);
        if (issueType) {
            return (
                <Popup
                    // @ts-ignore
    ...
    return (
        <>
            <li className={`drag-column drag-column${kanbanType.className ? '-' + kanbanType.className : ''}`}>
                            ...
                            {issues.map((issue, index) => (
                                <Draggable key={issue._id} draggableId={issue._id} index={index}>
                                    {(provided, snapshot) => (
                                        ...
                                        >
                                            <Popup
                                                trigger={<section className="top">{issue.summary}</section>}
                                                inverted
                                                position="bottom center"
                                                size="small"
                                            >
