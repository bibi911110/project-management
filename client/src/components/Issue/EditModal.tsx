...
import { Form, Modal, Message, Icon, Grid, Dimmer, Loader } from 'semantic-ui-react';
...
class EditModal extends React.Component<EditModalProps, EditModalState> {
    ...
    fetchIssue = async () => {
        ...
        const { assigneeId, description, summary, statusId, projectId, epicId, creatorId, issueTypeId, priorityId } =
            data;
        const issueType = this.state.issueTypes.find((type) => type._id === issueTypeId);
        const project = this.state.projects.find((project) => project._id === projectId);
        const user = this.state.assignees.find((user) => user._id === assigneeId);
        const priority = this.state.priorities.find((priority) => priority._id === priorityId);
        const kanbanType = this.state.kanbanTypes.find((kanban) => kanban._id === statusId);
        if (success) {
            this.setState({
                currentIssue: data,
                summary,
    ...
    fetchProjects = async () => {
        ...
        if (success) {
            const prepareData = data.map((project) => this.createProjectLabel(project));
            this.setState({
                projects: data,
    ...
    fetchIssueTypes = async () => {
        ...
        if (success) {
            const prepareData = data.map((issueType) => this.createIssueTypeLabel(issueType));
            this.setState({
    ...
    fetchPriorities = async () => {
        ...
        if (success) {
            const prepareData = data.map((priority) => this.createPriorityLabel(priority));
    ...
    IssueTypeIcon = () => {
        ...
        const issueType = issueTypes.find((type) => type._id === issueTypeLabel.value);
    ...
    render() {
        ...
        return (
            <>
                ...
                {!isLoading && (
                    <Modal open={isOpen} size="large" centered closeOnEscape onClose={this.handleClose}>
                        <Modal.Header className="edit-header">
                            <Form.Field id="issueType" name="issueType" control={this.IssueTypeSelect} required />
                            <div>
                            ...
                            <Modal.Description>
                                <Form>
                                    <Grid columns="2" stackable>
                                        ...
                                        <Grid.Column width="6">
                                            <Form.Field id="kanban" name="kanbanType" control={this.KanbanSelect} />
                                            <Form.Field
                                                id="assignee"
                                                name="assignee"
                                    ...
                                    <Message error header="Issue could not be updated" visible={this.state.error} />
                                </Form>
                            </Modal.Description>
                        </Modal.Content>
                    </Modal>
