...
const uploadAvatar = async (req, res) => {
    ...
    try {
        ...
        if (!checkDuplicate.length) {
            ...
            newAvatar
                .save()
                .then(() => res.status(200).json({ success: true, data: file, message: 'Avatar updated!' }))
                .catch((error) =>
                    res.status(400).json({
                        success: false,
                        error: error.message,
...
const deleteAvatar = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, id: avatar._id, message: 'Avatar deleted.' });
    } catch (error) {
        return res.status(400).json({ success: false, error, message: 'Avatar could not be deleted.' });
    }
};

const updateAvatar = async (req, res) => {
    return res.status(400).json({ success: false, message: 'Route not implemented.' });
};
